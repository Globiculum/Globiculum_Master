    ...
    const { result, attempts: totalAttempts } = await withRetry(
      ...
      },
      {
        ...
        shouldRetry: (error) => {
          ...
          if (msg.includes('validation') || msg.includes('unauthorized')) {
            return false;
          }
          // Don't retry rate-limit / quota errors - these are billing/plan limits,
          // not transient failures; retrying them only burns quota faster.
          if (msg.includes('429') || msg.includes('RATE_LIMITED') || msg.includes('quota') || msg.includes('RESOURCE_EXHAUSTED')) {
            return false;
          }
          // Retry timeouts, network errors, and transient server failures
